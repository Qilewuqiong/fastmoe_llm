...
from fmoe_llm.transformer import FMoETransformerMLP
...
class MegatronMLP(FMoETransformerMLP):
    ...
    def __init__(self, args, layer_idx, gate=None):
        ...
        if not args.balance_strategy or args.balance_strategy == "naive":
            ...
            from fmoe_llm.gates import NoisyGate
            gate = NoisyGate
        elif args.balance_strategy == "gshard":
            from fmoe_llm.gates import GShardGate
            gate = GShardGate
        elif args.balance_strategy == "switch":
            from fmoe_llm.gates import SwitchGate
            gate = SwitchGate
        elif args.balance_strategy == "swipe":
            from fmoe_llm.gates import SwipeGate
            gate = SwipeGate
        elif gate is None:
...
def fmoefy(
    ...
    * `distributed_expert` is set to True if different experts are located in
    ...
    if megatron_version == "v2.2":
        ...
        for idx, l in enumerate(model.transformer.layers):
