...
"""
import torch
from fmoe_llm.balance import reset_balance_profile
from fmoe_llm.balance import update_balance_profile
from fmoe_llm.utils import get_torch_default_comm
...
def add_balance_log(model, writer, iteration):
    ...
    while hasattr(model, "LlamaModule"):
        #model = model.module
        model = model.LlamaModule
    losses = [l.mlp.gate.get_loss(clear=True)
                for l in model[0].module.module.transformer.layers
