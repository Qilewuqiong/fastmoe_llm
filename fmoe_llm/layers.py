...
#            print("in FMoE, before flatten, inp: ", moe_inp.size())
...
#        if torch.distributed.get_rank() == 0:
#            print("in FMoE, before ensure_comm, inp: ", moe_inp.size())
...
#        if torch.distributed.get_rank() == 0:
#            print("in FMoE, before slice_func, inp: ", moe_inp.size())
...
#        if torch.distributed.get_rank() == 0:
#            print("in FMoE, before gate, inp: ", moe_inp.size())
...
#        if torch.distributed.get_rank() == 0:
#            print("moe after gate, inp: ", moe_inp.size())
...
#        if torch.distributed.get_rank() == 0:
#            print("mask: ", self.mask, ", mask_dict: ", self.mask_dict)
...
#        if torch.distributed.get_rank() == 0:
#            print("before bmm, gate_score: ", gate_score.size(), ", moe_outp: ", moe_outp.size())

        def bmm_func(tensor):
            dim = tensor.shape[-1]
            tensor = torch.bmm(gate_score, tensor).reshape(-1, dim)
            return tensor
#       print("start to perform bmm")
...
#            print("start to perform all_gather_func")
